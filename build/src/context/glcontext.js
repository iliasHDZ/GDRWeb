...
const buffer_array_builder_1 = require("./webgl2/buffer-array-builder");
...
const attributes = {
    ["aPos"]: buffer_array_builder_1.ArrayType.FLOAT2,
    ["aCol"]: buffer_array_builder_1.ArrayType.SHORT,
    ["aFlags"]: buffer_array_builder_1.ArrayType.SHORT,
    ["aHsv"]: buffer_array_builder_1.ArrayType.SHORT,
    ["_0"]: buffer_array_builder_1.ArrayType.SHORT,
    ["aTex"]: buffer_array_builder_1.ArrayType.FLOAT2,
    ["aGroups"]: buffer_array_builder_1.ArrayType.SHORT4,
    ["aSCp"]: buffer_array_builder_1.ArrayType.SHORT4
};
...
class WebGLContext extends context_1.RenderContext {
    ...
    */
    ...
    init() {
        ...
        this.quadShader = new program_1.ShaderProgram(gl);
        ...
        this.quad = new buffer_array_1.BufferArray(gl);
        this.quad.add(this.quadShader.attrib('aPos'), new buffer_1.BufferObject(gl, QUAD_VERTICES), 2, 0, 2 * 4);
    }
    setColorChannel(channel, color, blending) {
    ...
    updateInfoTexture(buffer, texture, width) {
        let gl = this.gl;
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, newBuffer);
    }
    updateFloatInfoTexture(buffer, texture, width) {
        let gl = this.gl;
        const height = Math.ceil(Math.ceil(buffer.length / 4) / width);
        ...
        for (; i < totalFloats; i++)
            newBuffer[i] = 0;
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32F, width, height, 0, gl.RGBA, gl.FLOAT, newBuffer);
    }
    ...
    updateColorInfoTexture() {
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 2, 1011, 0, gl.RGBA, gl.UNSIGNED_BYTE, buffer);
    }
    ...
    genQuadStructs(m, c, sprite, groups = [0, 0, 0, 0], hsvId) {
        groups = groups.slice();
        ...
        let t_l = crop.x, t_r = crop.x + crop.w, t_t = crop.y, t_b = crop.y + crop.h;
        let t;
        if (sprite.rotated) {
            t = [
    ...
    compileObjects(c) {
        const builder = new buffer_array_builder_1.BufferArrayBuilder(attributes);
