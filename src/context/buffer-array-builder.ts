...
export class BufferArrayBuilder {
    ...
    public createVertexArray(gl: WebGL2RenderingContext, program: ShaderProgram, buffer: BufferObject): VertexArray {
        const array  = new VertexArray(gl);
    ...
    public compile(gl: WebGL2RenderingContext, program: ShaderProgram): VertexArray {
        const buffer = BufferObject.fromData(gl, this.generateBuffer());
        return this.createVertexArray(gl, program, buffer);
    }
};